...
namespace minecraft {
                                  ...
                                  Memory_Arena         *arena)
    {
        ...
        bool loaded = load_asset(texture_asset_handle);
        assert(loaded);
        ...
        Rectanglei texture_rectangle = { 0, 0, texture->width, texture->height };

        for (u32 i = 0; i < sub_texture_count; i++)
        {
            Rectanglei *rectangle = sub_texture_rectangles + i;
            Assert(is_rectangle_inside_rectangle(*rectangle, texture_rectangle));
